...
int Ball::getRadius() { return m_ball.getRadius(); }

int Ball::getScore() { return m_score; }

int Ball::getLives() { return m_lives; }

void Ball::move(sf::Vector2u windowSize, Paddle &l_paddle, Wall &l_wall,
                float elapsed) {
...
void Ball::blockCollision(Wall &wall) {
  for (int i = 0; i < wall.getBlocks().size(); i++) {
    ...
    // bool leftBlockx = m_ball.getPosition().x >= block.getPosition().x -
    // getRadius(); bool rightBlockx = m_ball.getPosition().x + getRadius() <=
    // block.getPosition().x + block.getSize().x;
    bool bottomBlocky =
        m_ball.getPosition().y - block.getPosition().y + block.getSize().y <
        getRadius();
    bool topBlocky =
        block.getPosition().y - m_ball.getPosition().y < m_ball.getRadius();

    bool leftBlockx =
        m_ball.getPosition().x + getRadius() > block.getPosition().x;
    bool rightBlockx = m_ball.getPosition().x - getRadius() <
                       block.getPosition().x + block.getSize().x;

    if (!(leftBlockx && rightBlockx)) {
      continue;
    }
...
void Ball::windowCollision(sf::Vector2u windowSize) {
  if ((m_ball.getPosition().x > windowSize.x - getRadius() &&
       m_increment.x > 0) ||
      (m_ball.getPosition().x - getRadius() < 0 && m_increment.x < 0)) {
    m_increment.x = -m_increment.x;
  }
  if ((m_ball.getPosition().y - getRadius() < 0 && m_increment.y < 0)) {
    m_increment.y = -m_increment.y;
  } else if (m_ball.getPosition().y > windowSize.y - getRadius() &&
             m_increment.y > 0) {
    reset();
  }
}
...
void Ball::paddleCollision(Paddle &l_paddle) {
  ...
  if (leftPaddlex && rightPaddlex && paddley) {
    ...
    if (l_paddle.getPhysicalDirection() * m_increment.x < 0 &&
        l_paddle.getDirection() != Direction::None) {
      m_increment.x = -m_increment.x;
    }
  }
}

void Ball::pause() { m_increment = sf::Vector2f(0, 0); }

void Ball::resume() { m_increment = sf::Vector2f(175.f, 300.f); }
