...
TextBox::TextBox(int l_visible, int l_charSize, int l_width,
                 sf::Vector2f l_screenPos)
    : m_content(m_font) {
  setup(l_visible, l_charSize, l_width, l_screenPos);
}
//     : m_font("../Roboto-Regular.ttf"), m_content(m_font) {
//   setup(l_visible, l_charSize, l_width, l_screenPos);
// }

void TextBox::setup(int l_visible, int l_charSize, int l_width,
                    sf::Vector2f l_screenPos) {
  m_numVisible = l_visible;
  sf::Vector2f l_offset(2.0f, 2.0f);
  string font_name = "Roboto-Regular.ttf";
  while (!filesystem::exists(font_name)) {
    font_name = "../" + font_name;
  }
  m_font.openFromFile(font_name);
